...
namespace DemoCaseGui.Core.Application.ViewModels
{
    public class CaseViewModel : BaseViewModel, INotifyPropertyChanged
    {
        ...
        private async void TimerElapsed(object? sender, EventArgs args)
        {
            ...
            {
                DateTime = now,
                Value = Convert.ToDouble(MotorSpeed)
            });
            ChartUpdated?.Invoke();
            ...
            SetpointSpeed = (float?)_s7Client.GetTagValue("setpoint_speed_M");
            SetpointPosition = (float?)_s7Client.GetTagValue("setpoint_position_M");
        ...
        {
            _s7Client.WritePLC(_s7Client.GetTagAddress("setpoint_speed_M"), SetpointWriteSpeed);
        }

        public void WritePosition()
        {
            _s7Client.WritePLC(_s7Client.GetTagAddress("setpoint_position_M"), SetpointWritePosition);
        }
        ...
        public void ManualInverterStep()
        {
